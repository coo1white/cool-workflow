...
for (const app of canonicalApps) {
  ...
  assert.equal(summary.legacy, false);
    assert.equal(summary.version, "0.1.18");
  ...
  assert.equal(shown.app.id, app.id);
    assert.equal(shown.app.version, "0.1.18");
    assert.equal(shown.app.compatibility.minVersion, "0.1.18");
  ...
  assert.match(report, new RegExp(`Workflow App: ${app.id}@0\\.1\\.18`));
}
