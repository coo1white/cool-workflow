...
- **Capability**: Three trust/safety gaps are closed. A tail-truncation attack on the audit log (deleting the last N lines of `audit/events.jsonl`) used to leave `cw audit verify` green; `cw audit head` plus `--expect-head`/`--expect-count` now catch it. A manual result accept (`cw result`, `cw worker output`) used to slip past the `--require-attested-telemetry` gate when no delegation metadata was present at all; it is now blocked the same as an unattested one, with an audited `--allow-unattested` override. Two processes recording results for two different tasks of the same run could silently drop one task's completion; `state.json` is now locked for the whole read-modify-write cycle, not just the write. Separately, run state, cache keys, and eval-replay snapshots no longer depend on the host's locale — a machine with a different `LANG` used to sort task ids differently and quietly shift a cache key or an eval-replay comparison; every internal sort is now locale-pinned. `cw search` is now a real, declared capability (it silently worked before with no parity-table row), and three long-dead `dispatchLegacy` command stubs are gone.
- **Implementation**: `withRunStateLock` wraps the whole checkpoint cycle in `run-store.ts`; `core/util/collate.ts`'s `stableCompare` replaces every bare `localeCompare` across core/ and shell/ (53 sites); a new `trustAuditHead` projection and `TrustAuditAnchor` param back the anchor check; `worker-isolation.ts`'s gate fires on missing telemetry too. Structurally, `core/capability-table.ts` (4,300+ lines) is split into a pure `core/capability-data.ts` plus a `src/wiring/capability-table/` composition layer, and a new `purity:check` gate (`scripts/purity-gate.js` + a committed baseline) makes the core/shell import boundary a checked ratchet instead of an unenforced claim. A restored `test:unit` layer (152 pure-function tests, lost at the v2 cutover) runs alongside the black-box smoke and conformance suites.
- **Tests**: New `run-state-lock-concurrency-smoke.js` (real concurrent-process reproduction, 1/6 completions kept before the fix, 6/6 after), `collate-stablecompare.test.js` (cross-locale child-process proof), `trust-audit-anchor-smoke.js`, `dispatch-legacy-burndown-smoke.js`, and a new conformance case each for the telemetry gate and the locale migration. `npm run test:gate` (181 smokes), `test:unit` (153), and conformance (104/104) all green; `purity:check` and `release:check` pass.
- **Risk**: Low to moderate. The trust-gate and lock fixes are additive/fail-closed with no existing-output change on the default path; the locale migration is measured byte-identical to the old `localeCompare` under every locale tested so far, with an eval-replay conformance case proving the risk case (host locale change) end to end. The capability-table split is proven behaviorally byte-identical (a full `REGISTRY` dump diffed clean against the pre-split build) but is the largest structural change in this release — worth a careful look at `cw search`/`cw help`/`cw_audit_head` if anything in that area misbehaves. Zero new runtime dependencies.

## 0.2.1

- **Capability**: A failed agent hop now tells you why. Before, a failed `claude`/`gemini`/`opencode` worker left only a bare `claude exited 1` in `logs/agent-stderr.log` — no reason at all, because a real failure (not logged in, a rate limit, a relay error) is often reported by the agent on stdout, not on stderr, and that text was silently thrown away. `logs/agent-stderr.log` now carries the real reason on both the single-task and the parallel/concurrent dispatch paths. `architecture-review` also now answers the real `--question` asked in EVERY phase (Map, Assess, Verify), not only in 3 of 14 prompts — a question that is not a plain risk audit ("how does X work", "is it safe to change Y") now shapes what every lens looks for, not only the final verdict step. The shared `cw:result` contract and the README/wiki Quick Start examples use plain words for a "finding" instead of always saying "risk", so a non-security question does not get a risk-only answer shape forced onto it.
- **Implementation**: A new shared `buildFailureDetail()` in `agent-adapter-core.js` falls back to the agent's own parsed stdout text when real stderr is empty (`claude-p-agent.js`, `gemini-agent.js`, `opencode-agent.js`; `codex-agent.js` needs no change — it reads its answer from a file, not from parsed stdout). Separately, the SAME sandboxed spawn was dropping the `USER` environment variable, which a real vendor CLI needs to find its own login/keychain credential in headless mode — found live, after the stderr fix, by seeing the REAL reason for the first time ("Not logged in"). `USER` forwarding was fixed in TWO places that had drifted into duplicate copies of the same allowlist: the single-task path (`execution-backend/agent.ts`) and the parallel/concurrent dispatch path (`shell/drive.ts`) — both now call one shared `buildAgentChildEnv()` so this cannot silently drift into a third copy. `apps/architecture-review/workflow.js` threads `{{question}}`/`{{focus}}`/`{{invariant}}` into the 5 Map + 5 Assess + 1 Verify prompts that were fixed strings before (`map:server-api`, `assess:domain`, and `verdict:synthesis` already did this and are unchanged) — same 6 Map + 6 Assess + 1 Verify + 1 Verdict agents, same ids, same order, wording only. `agent-adapter-core.js`'s `RESULT_CONTRACT` field `title` is now "short finding title" instead of "short risk title" (used by all four vendor wrappers, so this applies project-wide); the README, its generated npm mirror, and `docs/wiki/Getting-Started.md` now mix a mechanism question, a risk question, and a decision question in their Quick Start examples instead of three risk-only ones.
- **Tests**: New `"auth-error"` shim cases in the `claude`/`gemini`/`opencode` wrapper smokes (a fake vendor binary reports its real failure on stdout with empty stderr; the persisted log now carries that reason instead of a bare exit code). New `agent-backend-user-env-smoke.js` (the single-task path) and `agent-backend-concurrent-user-env-smoke.js` (the parallel/concurrent path — drives the real `architecture-review` app's Map phase with a stub agent) both prove `USER` reaches a REAL spawned child; both verified red before their fix and green after. New `architecture-review-question-aware-smoke.js` asserts `{{question}}` appears in every Map/Assess/Verify prompt and pins the exact task-id list/order. `npm run test:gate` 178/178 pass; `npm run ci` (full release gate) green.
- **Risk**: Low. Every fix enriches a FAILURE-diagnostic path (a persisted log file, an internal env-construction helper) or rewords an internal prompt/contract string — no schema, exit code, task id, phase count, or `--json` shape changed. A pure risk question still gets the same 6 Map + 6 Assess + 1 Verify + 1 Verdict coverage as before, now told to relate its findings to the stated question instead of running blind. Zero new runtime dependencies.
