...
assert.equal(summary.candidateId, "dogfood-release-0.1.52");
...
assert.equal(state.workflow.app.version, "0.1.52");
assert.equal(state.inputs.repo, repoRoot);
assert.equal(state.inputs.version, "0.1.52");
...
// v0.1.52: the architecture-review agent-delegation dogfood, --smoke half (CI-
