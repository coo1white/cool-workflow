...
// A "both" row that has NOT had a cli binding attached (most of the 196
// mcp rows, at this point in the build) must carry mcp but no cli.
{
  const row = findCapability("app.run");
  assert.ok(row, "app.run must be declared");
  assert.equal(row.surface, "both", "app.run's declared surface field is still 'both' even though no cli is attached yet");
  assert.ok(row.mcp, "app.run must carry an mcp binding");
  assert.equal(row.cli, undefined, "app.run must carry no cli binding at this point in the build");
}
