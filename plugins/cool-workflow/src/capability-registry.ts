...
const BUILTIN_CAPABILITIES: CapabilityDescriptor[] = [
  ...
  { capability: "orphans.list", summary: "List run directories under .cw/runs/ that the run registry cannot see (no state.json — a killed/interrupted process never wrote one), with age + bytes. Read-only.", entry: "listOrphanRuns", surface: "both", cli: { path: ["orphans", "list"], caseTokens: ["orphans", "list"], jsonMode: "flag" }, mcp: { tool: "cw_orphans_list" } },
  { capability: "orphans.gc", summary: "Reclaim orphan run directories (no state.json): an age sweep (--min-age-minutes, default 60) or --all. Deletes only inside a scanned repo's .cw/runs/, never a run the registry knows about.", entry: "gcOrphanRuns", surface: "both", cli: { path: ["orphans", "gc"], caseTokens: ["orphans", "gc"], jsonMode: "flag" }, mcp: { tool: "cw_orphans_gc" }, payloadIdentical: false, reason: "Mutating: removes orphan run directories and reports now-derived freedBytes/removed; both surfaces perform the identical sweep." },
...
const PAYLOAD_PROBE_DEFERRED_GROUPS: Array<{ reason: string; capabilities: string[] }> = [
  {
    ...
    capabilities: [
      ...
      "clones.list",
      // orphans.list is payload-identical by construction (both surfaces call listOrphanRuns),
      // but it reads the external .cw/runs/ filesystem tree (absolute paths + per-entry
      // bytes/ages), so it is not byte-probed by the deterministic bootstrap either.
      "orphans.list",
      "app.init",
      "app.run",
      "migration.list",
