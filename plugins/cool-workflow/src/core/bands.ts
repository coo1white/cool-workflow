...
export type BandTier = "none" | "1" | "2" | "3";

export interface BandsTierRule {
  outcome: string;
}
...
export interface BandsConfig {
  ...
  tiers: { "1": BandsTierRule; "2": BandsTierRule; "3": BandsTierRule };
...
export interface BandsEvaluation {
  metric: string;
  window?: string;
...
function requireTierRule(value: unknown, tier: string): BandsTierRule {
 ...
 *  read is not. */
export function parseBandsConfig(raw: unknown): BandsConfig {
  if (!isPlainObject(raw)) throw new Error("bands config: must be a JSON object");
  ...
  if (!isPlainObject(raw.tiers)) throw new Error('bands config: "tiers" must be an object with keys "1", "2", "3"');
  const tiers = {
...
export function parseBandsInput(raw: unknown): BandsInput {
  if (!isPlainObject(raw)) throw new Error("bands input: must be a JSON object");
  ...
  if (hasValue) {
    if (!isFiniteNumber(raw.value)) throw new Error('bands input: "value" must be a finite number');
  ...
  return { series: [...(raw.series as number[])] };
}
...
function tierFor(sigmaMultiple: number): BandTier {
  if (sigmaMultiple >= 3) return "3";
  if (sigmaMultiple >= 2) return "2";
  if (sigmaMultiple >= 1) return "1";
  return "none";
}
...
export function evaluateBands(config: BandsConfig, input: BandsInput): BandsEvaluation {
  let observedValue: number;
  let observedStddev: number | undefined;
...
export function intentFileName(metric: string, now: string): string {
  return `${safeSegment(now)}-${safeSegment(metric)}.md`;
}

export interface IntentDoc {
  evaluation: BandsEvaluation;
  now: string;
  configDigest: string;
  inputDigest: string;
}
...
export function buildIntentMarkdown(doc: IntentDoc): string {
