...
The archive file itself is one JSON document: `{ schemaVersion: 1, exportedAt, sourceVersion, run, files[], integrity { fileCount, manifestSha256 }, trust? { publicKeyPem, algorithm: "ed25519" }, artifacts[], audit[] }`. Each `files[]` entry is `{ relativePath, role: "artifact"|"audit"|"telemetry"|"run-file", contentBase64, sha256, sizeBytes, sourcePath? }` (src/run-export.ts:23-32,117-138). `state.json`, `import-manifest.json`, and `*.lock` files are never packed; symlinks are skipped. Before any restore write, the file table makes separators portable and refuses an absolute path, traversal, control byte, unknown role, bad sha256, unsafe size, reserved path, or second row with the same normalized path. Files sort by code-point order on `relativePath`. The manifest digest hashes `{relativePath, role, sha256, sizeBytes}` rows in that order; `sourcePath` is left out on purpose.
