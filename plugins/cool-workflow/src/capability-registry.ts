...
const BUILTIN_CAPABILITIES: CapabilityDescriptor[] = [
  ...
  { capability: "ledger.list", summary: "Read + verify every entry in a shared ledger directory (fail-closed inbox).", entry: "listLedgerEntries", surface: "both", cli: { path: ["ledger", "list"], caseTokens: ["ledger", "list"], jsonMode: "default" }, mcp: { tool: "cw_ledger_list", requiredArgs: ["dir"] }, payloadIdentical: false, reason: "Output depends on the on-disk contents of the named ledger directory, which the generic payload probe does not populate. Both surfaces call the same listLedgerEntries core; ledger-verify-smoke covers the fail-closed inbox." }
];
