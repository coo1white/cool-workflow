...
export function reportToHtml(markdown: string, title = "Report"): string {
  ...
  const toneName = ({ PASS: "pass", BLOCKED: "warn", FAILED: "bad" } as Record<string, keyof typeof tones>)[verdict || ""];
  const stamp = verdict
    ...
    : "";
  const band =
    '<div class="flex h-[52px] items-center justify-between gap-5 bg-neutral px-8 text-[12px] text-neutral-content">' +
  ...
  return (
    `<!doctype html><html lang="en" dir="ltr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="theme-color" content="#161b22"><title>${escapeHtml(title)}</title><style>${REPORT_CSS}</style></head>` +
