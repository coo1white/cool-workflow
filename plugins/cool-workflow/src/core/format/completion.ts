...
export function formatCompletionScript(shell: string): string {
  const words = completionWords();
  if (shell === "bash") return bashScript(words);
  if (shell === "zsh") return zshScript(words);
  if (shell === "fish") return fishScript(words);
