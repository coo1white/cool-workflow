// Human formatting for the operator UX surface (CLI-only; never affects --json /
// MCP payloads). Pure functions — a summary/result object in, a string out —
// carved out of operator-ux.ts (FreeBSD-audit R-series, run-registry/format.ts
// template) so the operator module no longer bundles the rendering layer.
// Re-exported from operator-ux.ts to keep the public surface byte-unchanged.
import { TopologySummary, TrustAuditSummary } from "../types";
import { MultiAgentSummary } from "../multi-agent";
...
function operator(summary: OperatorRunSummary) { return summary.multiAgentOperator; }
