...
| 2026-06-14 | `commit --partial-task <id>`: wire the existing partial-commit path to an operator flag | Parked — NOT behavior-bearing. StateCommit.partial / partialTaskIds are SET (commit.ts) but have ZERO readers across src (grep-verified): no resume/report/gc logic scopes task state by them, so they are an unread metadata marker, and the pitched defer-and-resume-the-remainder semantics do not exist. Wiring a flag to set an unread field adds an operator surface with no behavioral effect — the same parsimony violation that dropped the candidate-scoring threshold knob. Revisit only if/when partial commit gains real task-scoping semantics (a bigger lifecycle change), at which point the flag has something to drive. |
