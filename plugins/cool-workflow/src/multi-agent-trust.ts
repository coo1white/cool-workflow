...
export function policyForRole(role: AgentRole): MultiAgentPolicy {
  ...
  return {
    ...
    allowedJudgeOperations: unique([
      ...(isJudge ? ["verdict", "rationale"] as const : []),
      ...(isChair ? ["rationale", "panel-decision"] as const : [])
    ]),
...
export function recordRolePolicyAudit(run: WorkflowRun, role: AgentRole): TrustAuditEvent {
  return recordTrustAuditEvent(run, {
    ...
    metadata: role.policy as unknown as Record<string, unknown>
  });
}

export function authorizeMultiAgentAction(
  run: WorkflowRun,
  input: {
    operation: MultiAgentPolicyOperation;
    actor?: BlackboardAuthor;
...
export function assertMultiAgentActionAllowed(run: WorkflowRun, input: Parameters<typeof authorizeMultiAgentAction>[1]): MultiAgentTrustDecision {
  const decision = authorizeMultiAgentAction(run, input);
  if (!decision.allowed) throw new Error(decision.reason);
  return decision;
}

export function recordBlackboardWriteAudit(
  run: WorkflowRun,
  input: {
    operation: MultiAgentPolicyOperation;
    status: string;
...
export function recordMessageProvenanceAudit(
  run: WorkflowRun,
  input: {
    messageId: string;
    topicId: string;
...
export function recordJudgeRationaleAudit(
  run: WorkflowRun,
  input: {
...
export function hasAcceptedJudgeRationale(
  run: WorkflowRun,
  input: { multiAgentRunId?: string; candidateId?: string; scoreId?: string } = {}
): boolean {
  return listTrustAuditEvents(run).some((event) =>
...
export function sourceForActor(actor?: BlackboardAuthor): "cw-validated" | "host-attested" | "operator-recorded" | "runtime-derived" {
...
function evaluatePolicy(policy: MultiAgentPolicy | undefined, operation: MultiAgentPolicyOperation, topicId: string | undefined, evidenceRefs: string[], evidence?: StateEvidence[]): string | undefined {
  ...
  if (operation.startsWith("candidate.")) {
    ...
    if (!policy.allowedCandidateOperations.includes(op as never)) return `candidate operation ${op} is outside policy ${policy.policyRef}`;
  } else if (operation.startsWith("judge.")) {
    const op = operation.slice("judge.".length);
    if (!policy.allowedJudgeOperations.includes(op as never)) return `judge operation ${op} is outside policy ${policy.policyRef}`;
  } else if (!policy.allowedWriteOperations.includes(operation as never)) {
 ...
 */
export function missingEvidence(policy: MultiAgentPolicy | undefined, operation: string, evidenceRefs: string[], evidence?: StateEvidence[]): string[] {
  if (!policy) return [];
  const required = unique([...(policy.requiredEvidenceFor?.[operation] || [])]);
  if (!required.length) return [];
  if (!evidenceRefs.length) return required;
  ...
  for (const need of required) {
    ...
    const matchIndex = refs.findIndex((ref, index) =>
      !consumed.has(index) &&
...
interface EvidenceRefView {
  /** Lowercased explicit kind tags for this ref (recordRef.kind, "kind:" prefix). */
  kinds: string[];
  /** Lowercased free-text fields a required label may be mentioned in. */
  haystacks: string[];
}
 ...
 */
function buildEvidenceRefIndex(evidenceRefs: string[], evidence?: StateEvidence[]): EvidenceRefView[] {
  const views: EvidenceRefView[] = evidenceRefs.map((ref, index) => {
...
function mergeEvidenceView(ref: string | undefined, record?: StateEvidence): EvidenceRefView {
  const kinds: string[] = [];
  const haystacks: string[] = [];
  const pushText = (value?: string) => {
    if (value) haystacks.push(value.toLowerCase());
  };
  // A "kind:<label>" locator prefix is an explicit, replay-stable kind tag.
  const taggedKind = (value?: string): string | undefined => {
  ...
  if (record?.recordRef?.kind) kinds.push(record.recordRef.kind.toLowerCase());
  pushText(ref);
