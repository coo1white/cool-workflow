...
export function reportToHtml(markdown: string, title = "Report"): string {
  ...
  const stamp = verdict
    ...
    : "";
  const band =
    '<div class="flex h-[52px] items-center justify-between gap-5 bg-[#14120f] px-8 text-[12px] text-[#f2ede4]">' +
  ...
  const prose =
    "prose prose-stone dark:prose-invert mx-auto max-w-[820px] px-6 pb-14 pt-8 " +
  ...
  return (
    `<!doctype html><html><head><meta charset="utf-8"><title>${escapeHtml(title)}</title><style>${REPORT_CSS}</style></head>` +
