...
import { mcpRequiredArgsForTool, mcpToolDefinition } from "./capability-registry";
...
function capabilityTool(capabilityId: string, description: string, properties: Record<string, unknown>): unknown;
function capabilityTool(
  ...
  if (typeof descriptionOrProperties === "string") {
    return mcpToolDefinition(capabilityId, descriptionOrProperties, maybeProperties ?? {});
  }
  return mcpToolDefinition(capabilityId, descriptionOrProperties);
}
