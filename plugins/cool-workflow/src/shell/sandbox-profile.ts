...
const BUNDLED_PROFILE_DEFINITIONS: SandboxProfileDefinition[] = [
  {
    ...
    description:
      "Preserves existing CW worker isolation: workers may read the workspace and write only accepted worker output paths unless additional allowedPaths are supplied.",
  ...
  },
  {
    ...
    title: "Locked Down",
    description:
      "Workers may read only their input and write only the primary result file. Command, network, and inherited environment access are denied by policy.",
    readPaths: ["$inputPath"],
    writePaths: [],
  ...
  if (profile && typeof profile.id === "string" && isBundledSandboxProfileId(profile.id)) {
    issues.push(
      issue(
        ...
        `Custom sandbox profile id "${profile.id}" is reserved (collides with a bundled profile); choose a different id`,
        absolutePath
      )
    );
    return { valid: false, profileFile: absolutePath, issues };
...
export function sandboxPolicyForWorker(profileId: string | undefined, context: SandboxResolutionContext): ResolvedSandboxPolicy {
...
export function validateSandboxWrite(policy: ResolvedSandboxPolicy, rawPath: string, workerId = ""): WorkerBoundaryViolation | null {
...
export function validateSandboxCommand(policy: ResolvedSandboxPolicy, command: string, workerId = ""): WorkerBoundaryViolation | null {
  ...
  if (policy.execute.mode === "none") {
    return denied(
      ...
      `Worker ${workerId} command execution is denied by sandbox profile ${policy.id}: ${normalized}`,
      undefined,
      effectiveSandboxWritePaths(policy)
    );
  }
  if (policy.execute.mode === "allowlist" && !(policy.execute.allow || []).includes(normalized)) {
    return denied(
      "sandbox-command-denied",
      `Worker ${workerId} command is outside sandbox profile ${policy.id}: ${normalized}`,
      undefined,
      effectiveSandboxWritePaths(policy)
    );
  }
...
export function validateSandboxNetwork(policy: ResolvedSandboxPolicy, target: string, workerId = ""): WorkerBoundaryViolation | null {
  ...
  if (policy.network.mode === "none") {
    return denied(
      ...
      `Worker ${workerId} network access is denied by sandbox profile ${policy.id}: ${normalized}`,
      undefined,
      effectiveSandboxWritePaths(policy)
    );
  }
  if (policy.network.mode === "allowlist" && !(policy.network.allow || []).includes(normalized)) {
    return denied(
      "sandbox-network-denied",
      `Worker ${workerId} network target is outside sandbox profile ${policy.id}: ${normalized}`,
      undefined,
      effectiveSandboxWritePaths(policy)
    );
  }
...
export function sandboxContextForRun(run: WorkflowRun): SandboxResolutionContext {
  return {
    ...
    customProfiles: run.customSandboxProfiles as Record<string, SandboxProfileDefinition> | undefined,
  };
}
...
export function persistCustomSandboxProfile(run: WorkflowRun, requested: string): void {
  ...
  const profiles = (run.customSandboxProfiles || {}) as Record<string, SandboxProfileDefinition>;
...
function normalizeWorkerOutput(policy: Partial<SandboxWorkerOutputPolicy> | undefined, context: SandboxResolutionContext): SandboxWorkerOutputPolicy {
  return {
