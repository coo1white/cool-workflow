...
//   Both release-flow.js --cut and CI's release.yml verify this exact path and
...
//   .cw-release/verdict-signing.pub is committed to the repo, release.yml,
...
function buildReviewerInput(resultPath) {
  ...
  return [
    ...
    "",
    "## Required output — your final message IS the verdict",
    "Do not write any file: your final message is saved as the verdict file",
    `(${resultDisplay}) for you, so a file you write yourself is never read.`,
    "The message is ONLY the verdict: no prose before the first line, no code",
    "fence, no summary after. First line MUST be exactly one of:",
    `  APPROVED ${HEAD}`,
...
// the tag, mirroring release.yml's HEAD-or-HEAD~1 tolerance: the verdict is
