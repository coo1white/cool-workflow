...
import { readJson } from "./fs-atomic";
import { CostPolicy, UsageRecord } from "./observability";
...
function normalizeUsage(raw: Record<string, unknown>, now: string): UsageRecord {
  ...
  const usage: UsageRecord = { schemaVersion: 1, source, attestedAt: typeof raw.attestedAt === "string" && raw.attestedAt ? raw.attestedAt : now };
...
export function parseUsageFromArgs(args: Record<string, unknown>, now: string): UsageRecord | undefined {
  ...
  if (input === undefined && output === undefined && total === undefined && model === undefined) return undefined;
