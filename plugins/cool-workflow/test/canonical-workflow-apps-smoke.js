...
for (const app of canonicalApps) {
  ...
  assert.equal(summary.version, "0.1.15");
  ...
  assert.equal(shown.app.version, "0.1.15");
  assert.equal(shown.app.compatibility.minVersion, "0.1.15");
  ...
  assert.equal(state.workflow.app.version, "0.1.15");
  ...
  assert.match(report, new RegExp(`Workflow App: ${app.id}@0\\.1\\.15`));
}
