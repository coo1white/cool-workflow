...
| 听到的说法 | 事实 |
|---|---|
| "CW 只能编排、执行依赖外部是个差距，应该内嵌 API" | 这是**定位红线**，不是差距。执行后端(现 `src/shell/execution-backend/` 目录)的红线不变——"CW DELEGATES"(见 `src/shell/pipeline-cli.ts` 的委托声明)。 |
...
| "blackboard 存在但没用起来" | **已深度接入**（`src/core/multi-agent/coordinator.ts`(v2 后约 800 行)，进了 dispatch / operator-ux / audit，落盘 `.cw/runs/<id>/blackboard/`）。要做的是用更狠，不是从零搭。 |
