...
<!-- archived from 2026-09-01-muse-spark-agent.md on 2026-09-07 -->
...
<!-- archived from 2026-09-01-muse-spark-agent.spec.md on 2026-09-07 -->
...
<!-- archived from 2026-09-01-sdlc-gap-closure.md on 2026-09-07 -->
...
<!-- archived from 2026-09-01-sdlc-gap-closure.spec.md on 2026-09-07 -->
...
<!-- archived from 2026-09-07-workbench-next.md on 2026-09-07 -->
...
Status: done 2026-09-07. The owner's later word ("留 Next 16.3.0，同时保留
cli，2 种功能都要") won over the earlier "stays plain HTML". Built under
the playbook program `sdlc/unify-js-architecture/` (its plan.md holds
the packets, the departures and the snapshot diff); merged as #679,
#681, #682, #683, #684, #685.
...
<!-- archived from 2026-09-25-pixelcrosshair-layout.md on 2026-09-25 -->
...
- "Unit tests are `test/*.test.js`" (`test/run-unit.js` header,
  `scripts/release-check.js`, `ci.yml`, `onramp.ts` comments): now
  `test/**/*.test.js` in folders that mirror `src/`; fixed in #705,
  with `test/README.md` and `AGENTS.md`.
- "Program plans live in root `sdlc/`" (`AGENTS.md` stack rule,
  `REVIEW.md` compliance pass, `release.yml` and `ci-shape-gate-smoke`
  comments): now `project/docs/sdlc/`; fixed in #707.
- The root had no written list; `ROOT_ENTRIES` in
  `test/root-layout-smoke.js` is it now (#706), and `project/docs/`
  has a map, `README.md` (#707).

## What this spec got wrong (recorded at close)

- PR order: the spec had PR 2 (docs) before PR 3 (root). #706 merged
  first, so #707 also dropped `sdlc` from `ROOT_ENTRIES`; the root
  smoke failed until it did, which is the check doing its job.
- The spec did not see that two signed `.cw-release/*.verdict` files
  have no final newline. The first style check flagged them; the fix was
  a byte-exact list shared by `.editorconfig` and the smoke, never a
  change to a signed file.
- A placeholder path (`src/core/state/x.ts`) in new comments failed
  `citation:check`; a real module path replaced it. Placeholders in
  comments must be real paths here.
- md budget: the spec said this file makes 134; with the docs map it
  peaked at 135/135, and archiving this file brings it back to 134.

## Status ledger

| Item | State | PR |
|---|---|---|
| Intent + spec (this file) | closed, archived by #707 | #705 |
| PR 1: tests by folder | merged | #705 |
| PR 2: one docs tree | merged with this ledger | #707 |
| PR 3: a tidy root | merged | #706 |

---

<!-- archived from 2026-09-26-perf-ratchets.md on 2026-09-26 -->
...
- `project/docs/benchmark.md` "Framework Overhead" said CW's overhead
  is constant. True against agent delay; not against worker count
  (bytes written grow as N squared). Fixed there, with the numbers.
- `AGENTS.md` "Gates & repo mechanics" had no word on the new gate. It
  now names the count ratchet and how to lower a ceiling.
- `manifest/source-context-profiles.json`: the runtime guard is
  50,300 lines, not 50,250 (operator's decision, 2026-09-26, #711).
- `src/shell/operator-ux.ts` is no longer where the no-run advice
  lives; it re-exports it from `src/core/format/recovery-hint.ts`.

## What this spec got wrong (recorded at close)

- PR 1 planned a fixture under `test/fixtures/apps/fan/`. The bench
  makes the app in its temp dir instead, since `scripts/` ships and
  `test/` does not.
- PR 2 was to cut round trips by one third. Each load cost three JSON
  passes (parse, then the copy's stringify and parse), so the cut was
  one half: 143 to 71.
- PR 3 planned a new `src/shell/git-head.ts`. The ratchet refused it
  (drive.modules 114 to 115), so the code went into `commit.ts`. The
  gate caught a cost the plan did not see: good.
- The plan did not see the runtime source-context guard: steps 2-4 add
  51 lines and the guard had 30 to spare. The operator raised it by 50.
- The acceptance line compared against 3340 ms, one unpaired run. Unpaired
  runs of the same build moved from 3340 to 4017 ms on this machine, so
  the check at close is paired and alternating (9 pairs), not against a
  fixed number.
- `cw doctor` (45 modules) and `cw app list` (46) were left as they
  are: doctor uses what it loads, and app list has to run each app's
  `workflow.js`. `cw version` and `cw help` (30) are the capability
  table itself; making that lazy is a bigger change for another intent.
- PR 4's plan did not see the onramp contract: a change under
  `src/wiring/capability-table/` must update a shipped doc in the same
  diff. Its first CI run failed on that; `docs/operator-ux.7.md` now
  states the no-run path. `release:check --skip-tests` belongs in the
  local checks before any push that touches the capability table.
- What is left of CW's own time at N=64 (about 3 s) is the whole-state
  rewrite on every step: fsync, open, stringify. The append-only state
  journal (Paths weighed) is the next step if that cost matters.

## Status ledger

| Item | State | PR |
|---|---|---|
| Intent + spec + plan (this file) | closed, archived by #713 | #708 |
| PR 1 — bench and ratchet | merged | #709 |
| PR 2 — parse the state once per load | merged | #710 |
| PR 3 — read HEAD without starting git | merged | #711 |
| PR 4 — cold start | merged | #712 |
| PR 5 — closing ledger | merged with this ledger | #713 |

Receipt: `project/docs/audits/perf-ratchets-receipt-2026-09-26.json`.
Leading measures: 20 minutes from this intent (#708) to the first
build PR (#709); 4 of 5 program PRs green on their first push (#712 needed a
second, for a doc).

---

<!-- archived from 2026-09-26-state-reads.md on 2026-09-26 -->
...
- `project/docs/rebuild/SPEC/pipeline-run.md` "Resume after interrupt"
  said every drive step re-loads the run from `state.json`. True of
  the old build; now a new drive call reads it, and a round inside one
  call reads it only when the file's stamp has moved. A line saying so
  is added there; resume itself is unchanged.
- `manifest/source-context-profiles.json`: the runtime guard is 60,000
  lines and the core guard 64,000, not 50,300 and 54,000 (operator's
  decision, 2026-09-26, #716). Runtime stays tighter than core.
- `src/shell/run-store.ts` "The ONLY place state.json is read or
  written" still holds: the reuse hands back an object this process
  wrote there, and never reads or writes the file by another path.
- The docs that call `state.json` the single source of truth still
  hold: every save writes it whole, as before.

## What this spec got wrong (recorded at close)

- The spec said a reused run is deep-equal to a load. It was not: the
  first forced-verify sweep found every reuse off by keys whose value
  is `undefined` in memory (JSON drops them, so a load never has
  them). `jsonShaped` in `run-store.ts` deletes them and refuses
  anything JSON would change; the sweep then found 414 of 414 equal.
  PR 1's probe had compared JSON text, which cannot see this; a strict
  compare is the right check for reuse.
- The plan did not see the line guards again: #716 put the runtime at
  50,349 against a cap of 50,300. The operator raised it to 60,000;
  the smoke also needs every subprofile tighter than core, so core went
  from 54,000 to 64,000 in the same PR.
- PR 1's plan named the serial read count "4 -> 3"; right, but the
  smoke's "guard reverted" figure had to be measured again (5, not 4).
- A probe patched into `dist/` survived one `npm run build`: the
  tsc incremental cache saw no source change and wrote nothing. After
  any probe in `dist/`, delete `.cache/tsconfig.tsbuildinfo` before
  the build that the counts and the commit come from.
- The acceptance line is met at 64 workers (-11.8%); at 16 workers the
  change is inside the noise, since the state there is small.
- The write side (0.96 s at 64 workers) is as it was. A journal is
  still turned down on the numbers in "Paths weighed"; the next cost to
  measure is the trust-audit append bucket (0.79 s at 64 workers,
  linear), mostly lock-file opens and fsyncs.

## Status ledger

| Item | State | PR |
|---|---|---|
| Intent + spec + plan (this file) | closed, archived by #717 | #714 |
| PR 1 — progress line uses the round's run | merged | #715 |
| PR 2 — seed the next round from the last save | merged | #716 |
| PR 3 — closing ledger | merged with this ledger | #717 |

Receipt: `project/docs/audits/state-reads-receipt-2026-09-26.json`.
Leading measures: 19 minutes from this intent (#714) to the first
build PR (#715); every program PR (#714, #715, #716)
green on its first push, one commit each.
Found on the way, logged in `project/docs/BACKLOG.md`: a write by a
non-drive writer during a drive round is lost at the round's save.

---

<!-- archived from 2026-09-26-audit-batches.md on 2026-09-26 -->
...
- `project/docs/benchmark.md` said the dispatch and settlement groups
  were joined for the concurrent work only. The serial path now does the
  same (3 audit appends a worker); a sentence says so there.
- `withTrustAuditBatch`'s comment said the lines are appended "before
  the caller checkpoints". Now also: they are written when the body
  throws, and `saveCheckpoint` writes an open batch's events before the
  state, so audit-before-state holds on every path.
- The man pages' "durable audit append" still holds: every audit event
  is fsynced before the checkpoint that follows it.

## What this spec got wrong (recorded at close)

- The spec did not see that a batch body can save a checkpoint:
  `recordWorkerFailure` (inside the accept group, on a sandbox
  violation) calls `saveCheckpoint`, which inside a batch would put the
  state on disk ahead of its audit events. The concurrent path already
  did this. #720 added `flushPendingTrustAudit`, called first in
  `saveCheckpoint`: an open batch's events are written, the batch stays
  open. The test for it replaced the planned "sandbox-denied result on
  the serial path" case, which the smoke and conformance suites cover.
- The spec's counts were 64 lower (238 -> 174, 430 -> 366); the ratchet
  went 80 lower (238 -> 158, 430 -> 350). Its stub agent returns no
  evidence, so each accept also records `worker.capture-warning`: 8
  events a worker, still 3 appends.
- The first estimate of this cost (0.79 s at 64 workers, in the
  state-reads close) counted work run inside audit calls; the audit's own
  fs time was 406 ms. The saving came out larger than that bucket alone
  (-15% of CW self time, about 390 ms at 64 workers) because a batch
  also skips the lock, tail-cache and last-byte reads of each event.
- A fsync counter in the new test counted recycled fd numbers at first
  (42 fsyncs for 28 events); it now forgets an fd on close.

## Status ledger

| Item | State | PR |
|---|---|---|
| Intent + spec + plan (this file) | closed, archived by #721 | #718 |
| PR 1 — the batch flushes on a throw too | merged | #719 |
| PR 2 — the serial path batches its groups | merged | #720 |
| PR 3 — closing ledger | merged with this ledger | #721 |

Receipt: `project/docs/audits/audit-batches-receipt-2026-09-26.json`.
...
<!-- archived from 2026-09-26-state-parts.md on 2026-09-26 -->
...
- `BACKLOG.md`'s whole-state-write row said no path keeps `state.json`
  byte-identical without touching a frozen surface, and named freezing
  records as the first way to open it. The row now says the checked
  cache did it with no frozen surface touched; the other two ways stay
  open there.
- `benchmark.md`'s ratchet section names "whole-state JSON round
  trips" as a count. It still is one, but a save no longer makes a
  round trip; a sentence there says what the count holds now.
- `docs/durable-state-and-locking.7.md` still holds as written: the
  write is a temp file, fsync, rename and directory fsync; only the way
  the bytes reach the temp file (`writev` of parts) changed.
- The state-writes part of this archive (above) keeps its words as
  they were: it was true of the paths weighed then.

## What this spec got wrong (recorded at close)

- The first build joined the kept texts into one string and was slower
  than `main`; the win needed the file written from parts. The spec
  was written after that was found, so the plan held, but the idea
  that a cache alone saves the stringify cost was wrong: the copy costs
  nearly as much.
- The operator's choice was "records immutable + cache". The measure
  found that a check per element gives the same saving with no frozen
  surface edited and no new way to throw; freezing was not built.
- `drive.stateRoundTrips` counts calls, not bytes: a save now counts 0
  though it still writes every byte and still stringifies each changed
  element. The count fell to 3; the time is in the receipt.

## Status ledger

| Item | State | PR |
|---|---|---|
| Intent + spec + plan (this file) | merged | #723 |
| PR 1 — write state.json from kept parts | merged | #724 |
| PR 2 — closing ledger | merged with this ledger | #725 |

Receipt: `project/docs/audits/state-parts-receipt-2026-09-26.json`.
