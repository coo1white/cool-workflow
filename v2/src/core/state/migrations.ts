...
import { REQUIRED_ARRAY_KEYS, REQUIRED_RECORD_KEYS, REQUIRED_TOP_LEVEL_KEYS } from "./schema";
...
export const RUN_STATE_MIGRATIONS: StateMigrationStep[] = [
  {
    ...
    migrate(state, context) {
      setDefault(
        ...
        CURRENT_RUN_STATE_SCHEMA_VERSION,
        context,
        "legacy run state did not declare schemaVersion"
      );
    },
    reverse(state, context) {
      if (state.schemaVersion === CURRENT_RUN_STATE_SCHEMA_VERSION) {
        ...
        context.changes.push({
          path: "schemaVersion",
          before: CURRENT_RUN_STATE_SCHEMA_VERSION,
          after: undefined,
...
export function findMigrationPath(
  ...
  const forward = new Map<number, MigrationPathStep[]>();
  const reverse = new Map<number, MigrationPathStep[]>();
...
export function migrateRunState(
  ...
  const report: StateMigrationReport = {
    ...
    supportedSchemaVersions: { min: MIN_SUPPORTED_RUN_STATE_SCHEMA_VERSION, max: CURRENT_RUN_STATE_SCHEMA_VERSION },
  ...
  for (const step of resolved.path) {
    if (step.reverse) step.edge.reverse!(state, context);
    else step.edge.migrate(state, context);
  }
...
function normalizeRunState(state: Record<string, unknown>, context: StateMigrationContext): void {
  ...
  if (!isRecord(state.multiAgent)) {
    ...
    setValue(
      state,
      "multiAgent",
      { schemaVersion: 1, runs: [], roles: [], groups: [], memberships: [], fanouts: [], fanins: [] },
      context,
      "multiAgent state is required"
    );
  } else {
  ...
  if (!isRecord(state.blackboard)) {
    ...
    setValue(
      state,
      "blackboard",
      { schemaVersion: 1, boards: [], topics: [], messages: [], contexts: [], artifacts: [], snapshots: [], decisions: [] },
      context,
      "blackboard state is required"
    );
  } else {
