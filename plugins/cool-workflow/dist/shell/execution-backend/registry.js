...
function sandboxGuaranteeLabels(attestation) {
    const labels = {
        read: "absent",
        write: "absent",
        command: "absent",
    ...
    if (!attestation)
        return labels;
    if (attestation.guarantees)
        return { ...labels, ...attestation.guarantees };
    for (const dimension of attestation.enforced || [])
        labels[dimension] = "enforced";
    for (const dimension of attestation.attested || [])
        labels[dimension] = "attested";
    return labels;
}
// ---------------------------------------------------------------------------
// Readiness probe.
