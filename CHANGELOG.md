...
- **Capability**: Cool Workflow's runtime is a from-scratch rebuild, and this release cuts it over as the shipping product. Every command behaves exactly as before — the 101-case black-box conformance suite (real `cw` invocations, byte-compared) holds 101/101 through the whole cutover — so the CLI output, `--json` machine surfaces, exit codes, state format, MCP tools, and signing are unchanged. What changed is the inside: the code is now a pure `core/` (no IO) + impure `shell/` (all file/process IO) split, and a single generic `capability-table` drives BOTH `cli/dispatch` and `mcp/dispatch` from one declaration per capability, so a CLI verb and its `cw_<tool>` MCP peer can never drift. As part of finishing the cutover, the CLI surface was completed: ~50 verbs that had an MCP tool but no CLI binding (`cw schedule *`, `cw routine *`, `cw queue *`, `cw sched *`, `cw run export|import|restore`, `cw gc run`, `cw registry show`, the `cw multi-agent role|group|fanout|fanin` reads, and more) are now bound on both surfaces.
- **Implementation**: The v2 tree (its own SPEC, PLAN, `src/core` + `src/shell`, and a permanent 101-case conformance suite) was swapped into `plugins/cool-workflow` in place, then driven to real feature parity: every behavior the black-box sample never exercised was ported from the old flat build into the core/shell split (remote-source SSRF/zip-slip/tar-slip guards, the whole multi-agent / blackboard / topology + MCP subsystem, the operator feedback lifecycle, sandbox H7 custom-profile persistence, bundle sealing, acceptance rationale, sub-workflow nesting, state-explosion compaction, schema-checked result intake, the CandidateScore read guard, `cw next`, the `worker output` RunSummary wrapper, and the 48 remaining CLI verb bindings). Two real bugs surfaced during the cutover and were fixed: `cw review policy` silently dropped its `--required-approvals` / `--authorized-roles` / `--allow-self-approval` flags (the handler read only the camelCase MCP keys, not the CLI's kebab-case), and `cw quickstart --bundle` did not exit non-zero when the sealed bundle failed self-verification. All release-gate tooling (dogfood, parity-check, sync-project-index, version-sync, onramp, run-state-schema, bump-version, telemetry attest-wrap) was repointed to the v2 layout.
- **Tests**: The full white-box smoke suite — 173 smokes importing v2's internals — is **173/173 green**, in addition to the **101/101** black-box conformance suite (which held 101/101 after every change, with zero regressions across the whole effort). The entire `npm run release:check` gate passes (13/13: docs presence, dist freshness, type check, onramp contract, run-state schema, tests, canonical apps, golden path, CLI-MCP parity, vendor manifest sync, version sync, project index sync, readme sync), and CI is green across the Node 18 + 22 × x86_64 + ARM64 matrix. The `v2/conformance/` suite is kept as a permanent CI regression net.
- **Risk**: Higher-touch than a normal release — the entire runtime was rebuilt — but tightly gated: the 101-case conformance suite pins byte-identical external behavior and stayed 101/101 throughout, the 173-smoke white-box suite locks feature-completeness, and the full release gate + multi-arch CI are green. Zero new runtime dependencies. The public CLI/MCP surface, `--json` data channel, signing, and state format are unchanged; the multi-agent red line (nothing callable crosses the executor boundary) is now welded into the type layer as well.
