...
export function hostRun(run: WorkflowRun, options: Record<string, unknown> = {}): MultiAgentHostResponse {
  const topologyId = stringOption(options.topology || options.topologyId || options.id) as MultiAgentTopologyId | undefined;
  ...
  const topologyRun = existing || applyTopology(run, topologyId, {
...
export function hostStep(run: WorkflowRun, options: Record<string, unknown> = {}): MultiAgentHostResponse {
  ...
  if (faninPlan) {
    const fanin = collectAgentFanin(run, faninPlan);
  ...
  if (scorePlan) {
    ...
    recordCoordinatorDecision(run, {
      ...
      evidenceRefs: score.evidence.map((entry) => entry.locator || entry.summary || entry.id).filter(Boolean) as string[],
...
export function hostBlackboard(run: WorkflowRun, action: string | undefined, options: Record<string, unknown> = {}): MultiAgentHostResponse {
  ...
  switch (action || "summary") {
    ...
    case "post":
    case "message":
      data = postBlackboardMessage(run, {
    ...
    case "add-artifact":
    case "artifact":
      data = addBlackboardArtifact(run, {
    ...
    case "context":
    case "put-context":
      data = putBlackboardContext(run, {
        blackboardId,
        topicId: requireString(topicId, "topic id"),
        kind: requireString(options.kind, "context kind") as never,
...
export function hostScore(run: WorkflowRun, options: Record<string, unknown> = {}): MultiAgentHostResponse {
  ...
  if (!candidate && stringOption(options.worker || options.workerId)) {
    candidate = registerCandidate(run, {
...
function classifyHostState(run: WorkflowRun): HostRunState {
  ...
  if ((run.candidates || []).some((candidate) => candidate.status === "registered")) return "ready-for-scoring";
...
function nextDispatchPlan(run: WorkflowRun, topology: MultiAgentTopologyRun | undefined, options: Record<string, unknown>) {
...
function nextFaninPlan(run: WorkflowRun, topology: MultiAgentTopologyRun | undefined) {
  ...
  if (!memberships.length || memberships.some((entry) => entry.status !== "reported" && !entry.verifierNodeId)) return undefined;
...
function nextSelectionPlan(run: WorkflowRun, options: Record<string, unknown>): { candidate: CandidateRecord; scoreId?: string } | undefined {
  const candidate = resolveCandidate(run, options) || topRankedCandidate(run);
  if (!candidate || (run.candidateSelections || []).some((entry) => entry.candidateId === candidate.id)) return undefined;
...
function activeBlackboardId(run: WorkflowRun, topology: MultiAgentTopologyRun): string | undefined {
