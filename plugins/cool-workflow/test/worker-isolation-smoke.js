...
const workerDirEntries = fs.readdirSync(manual.workerDir, { withFileTypes: true });
assert.ok(workerDirEntries.every((e) => !e.isDirectory() || fs.readdirSync(path.join(manual.workerDir, e.name)).length > 0), "worker folder must hold no empty directory");
