...
export interface RunResumeResult {
  ...
  drive?: {
    ...
    steps: Array<{ action: string; status: string; taskId?: string; reason?: string }>;
...
// ReclaimedOverlay's minimal shape needed here (full type lives in
// reclamation-io.ts; duplicated narrowly to avoid a circular import — this
// file only ever READS the tombstone list to derive tier/capability).
export interface ReclaimedOverlayLite {
  schemaVersion: 1;
  runId: string;
  tombstones: Array<{
    reclaimedAt: string;
...
export function isRunLifecycleState(value: unknown): value is RunLifecycleState {
  return (
    typeof value === "string" &&
    (["queued", "running", "blocked", "completed", "failed", "archived", "reclaimed"] as string[]).includes(value)
  );
}
...
export function loadReclaimedFromDir(runDir: string): ReclaimedOverlayLite {
...
function fingerprintRun(run: WorkflowRun): string {
  ...
  for (const fb of [...((run.feedback as Array<{ id: string; status: string }> | undefined) || [])].sort((a, b) =>
...
// ---------------------------------------------------------------------------
// Lifecycle classification (pure; byte-exact port of deriveLifecycle)
// ---------------------------------------------------------------------------
...
function lifecycleInputs(run: WorkflowRun): LifecycleInputs {
  ...
  return {
    ...
    openFeedback: ((run.feedback as Array<{ status: string }> | undefined) || []).filter(
...
/** Minimal contract needed to CREATE a new run for rerun. */
export interface RunPlanner {
  plan(appId: string, options: Record<string, unknown>): WorkflowRun;
}

function requireOverlayObject<T>(parsed: unknown, file: string): T {
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw new Error(
      `Corrupt overlay ${file}: expected a JSON object, got ${Array.isArray(parsed) ? "array" : parsed === null ? "null" : typeof parsed}`
    );
  }
  return parsed as T;
}
...
export class RunRegistry {
  ...
  private loadArchiveOverlay(repo: string): ArchiveOverlay {
    ...
    const parsed = requireOverlayObject<ArchiveOverlay>(readJson(file), file);
  ...
  private loadProvenanceOverlay(repo: string): ProvenanceOverlay {
    ...
    const parsed = requireOverlayObject<ProvenanceOverlay>(readJson(file), file);
  ...
  private loadRepos(): ReposFile {
    ...
    const parsed = requireOverlayObject<ReposFile>(readJson(file), file);
  ...
  queueDrain(options: { limit?: number; repo?: string } = {}): { schemaVersion: 1; drained: RunQueueEntry[]; remaining: number } {
  ...
  private deriveRecord(repo: string, runDir: string, overlays: RepoOverlays = this.loadRepoOverlays(repo)): RunRecord | null {
    ...
    try {
      ...
      assertNotSuspectedDataLoss(path.basename(runDir), result);
      run = result.run;
    } catch {
      return null;
    }
    ...
    const capabilityReason: RunCapabilityReason = lastTombstone
      ...
      : archive
        ? "archived-full"
        : "live-full";
    const appMeta = run.workflow.app as { id?: string; version?: string } | undefined;
    return {
      schemaVersion: 1,
      runId: run.id,
      appId: appMeta?.id,
  ...
  buildIndex(scope: "repo" | "home", reuse?: { repo: string; records: RunRecord[] }): RunRegistryIndex {
  ...
  private deriveRecordForRun(repo: string, runId: string): RunRecord | null {
    ...
    try {
      ...
      if (result.report.status === "unsupported") {
        throw new Error(
          `Run state for ${runId} is corrupt (fail closed): ${result.report.errors.join("; ") || "unsupported run state"}. Restore ${statePath} from a backup, or remove the run directory to start over.`
        );
      }
  ...
  archive(
    ...
    const record = this.deriveRecordForRun(repo, runId);
    if (!record) throw new Error(`Cannot archive: run ${runId} state became unavailable while archiving (fail closed).`);
    return {
      runId,
      repo,
  ...
  rerun(runId: string, options: { reason?: string; scope?: "repo" | "home" } = {}): RunRerunResult {
    ...
    const appMeta = originalRun.workflow.app as { id?: string } | undefined;
    ...
    const newAppMeta = newRun.workflow.app as { id?: string } | undefined;
    return {
      schemaVersion: 1,
      originalRunId: runId,
...
export function formatRegistryReport(report: RunRegistryReport): string {
  ...
  lines.push(
    `Freshness: ${report.freshness.status}${report.freshness.staleRuns.length ? ` (stale: ${report.freshness.staleRuns.join(", ")})` : ""}${
      report.freshness.missingRuns.length ? ` (missing: ${report.freshness.missingRuns.join(", ")})` : ""
    }`
  );
  ...
  if (r.tier && r.tier !== "live") {
    lines.push(
      `  tier=${r.tier} capability=${r.capability} reason=${r.capabilityReason}${r.reclaimedBytes ? ` bytesFreed=${r.reclaimedBytes}` : ""}${
        r.tombstoneHash ? ` tombstone=${r.tombstoneHash.slice(0, 19)}` : ""
      }`
    );
  }
  return lines.join("\n");
...
export function formatResume(result: RunResumeResult): string {
  ...
  if (result.drive) {
    lines.push(
      `  drive=${result.drive.status} workers=${result.drive.completedWorkers}/${result.drive.plannedWorkers} parked=${result.drive.parkedWorkers}`
    );
    for (const step of result.drive.steps) {
      lines.push(
        `  step ${step.action} [${step.status}]${step.taskId ? ` task=${step.taskId}` : ""}${step.reason ? ` reason=${step.reason}` : ""}`
      );
    }
    if (result.drive.commitId) lines.push(`  commit=${result.drive.commitId}`);
  }
