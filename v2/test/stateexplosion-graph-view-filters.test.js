...
// "candidate" view keeps only its named kinds (candidate/score/selection/worker/agent-fanin/root).
{
  ...
  const ids = record.nodes.map((n) => n.id).sort();
  assert.deepEqual(ids, ["candidate1", "worker1"], "candidate view keeps candidate and worker kinds, excludes task/commit/topic");
}
...
// "blackboard" view keeps only blackboard-* kinds (+ root/coordinator-decision).
{
  ...
  const ids = record.nodes.map((n) => n.id).sort();
  assert.deepEqual(ids, ["topic1"], "blackboard view keeps only blackboard-topic, excludes task/candidate/commit/worker");
}

// "topology" view: none of the mixedGraph kinds match, so only nothing is kept for it (no root injected here, unlike failures).
{
  const record = buildCompactGraphFromView("run-1", mixedGraph(), "topology", { now: "x" });
  assert.deepEqual(record.nodes, [], "topology view with no topology-kind nodes present keeps nothing");
}
