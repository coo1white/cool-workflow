...
- **Capability**: Restore an exported run on another machine in one fail-closed step — `cw run restore <archive>`. `cw run import` brings a run in and *runs* the same verification `verify-import` does (file digests + telemetry-ledger + trust-audit hash chains) but **does not fail on it** — it exits 0 even when that chain does not verify, so a tampered or corrupt run can be imported with a silent success. `run restore` reuses that verdict and **fails closed**: it integrity-inspects the archive first (refusing a corrupt bundle without writing anything), imports, and exits non-zero with `ok:false` on any run whose verification fails — so `cw run restore <archive> && proceed` can never act on an unverified run. Available on both surfaces (CLI `run restore`, MCP `cw_run_restore`).
- **Implementation**: A thin fail-closed composition in `capability-core.ts` (`runRestoreArchive`) over the existing `inspectArchive` + `importRun` + the verification `importRun` already returns (`ImportResult.verification`) — no new crypto/IO, no second verify pass. The CLI `run restore` case sets exit 1 on `!ok` (mirroring `inspect-archive`); the MCP `cw_run_restore` tool routes through the same core, kept in step by the CLI↔MCP parity gate. `run import`/`verify-import`/`inspect-archive` are byte-identical (POLA: the new verb is purely additive). This release also lands a large internal restructuring with **no behavior change**: the 1,163-line CLI `command-surface.ts` god-dispatch is decomposed into 16 per-family handler modules under `src/cli/handlers/` (multi-agent, run, collaboration, blackboard, coordinator, eval, node, gc/telemetry/demo, the operational families, candidate, …), each a byte-identical code-move; and the app-management methods move out of the `CoolWorkflowRunner` facade into `src/orchestrator/app-operations.ts`, completing the documented router pattern. `command-surface.ts` drops from 1,163 to 437 lines with every public CLI/MCP surface and output byte-exact (parity gate green throughout).
- **Tests**: `run-restore-failclosed-smoke` (NEW) builds an archive `run import` accepts (file + manifest digests valid) but whose telemetry-ledger hash chain fails verification, and asserts the contrast on the same archive — `run import` exits 0 with `verification.ok:false` while `run restore` exits non-zero with `ok:false` — plus a corrupt-bytes archive refused inspect-first (nothing written) and a happy verified restore; the fail-close is mutation-proven (deleting the exit-1 turns the smoke red). The decomposition shipped as nine dev-loop PRs (each plan → build → independent adversarial audit → PR → CI → squash-merge), every carve byte-identical and guarded by an extended family smoke plus the CLI/MCP parity gate. Full suite green.
- **Risk**: Low. `run restore` is a new opt-in verb; the existing run-archive verbs are byte-identical. The CLI/orchestrator restructuring is a pure code-move proven byte-identical by the parity gate, the full smoke suite, and per-cycle adversarial audits; no machine surface (`cw:result`, `--json`), signing, or schema red line was touched. Zero new runtime dependencies.
