...
export const CURATED_SMOKE_MAP: Array<{ patterns: string[]; smokes: string[] }> = [
  {
 ...
 *  this module. */
export function isCommentOnlyPatch(patchText: string): boolean {
  let sawLine = false;
...
export function resolveChangedFiles(options: ResolveChangedFilesOptions = {}): OnrampChangedFiles {
  ...
  for (const file of gitLinesOrThrow(root, ["diff", "--name-only", baseRef, "--"])) {
    ...
    if (/\.(ts|js|mjs)$/.test(file)) candidates.set(normalized, file);
  }
...
export function evaluateOnrampContract(
  ...
  if (
    (runtimeFiles.length > 0 || appFiles.length > 0) &&
    smokeFiles.length === 0 &&
...
function resolveBaseRef(root: string, changedFrom: string | undefined, env: NodeJS.ProcessEnv): string {
  ...
  if (env.GITHUB_BASE_REF) {
    ...
    throw new Error(
      `onramp: cannot resolve a base ref -- git merge-base HEAD ${baseBranch} found no common commit (fail closed, not treated as zero changes). Fetch the base branch with a full (non-shallow) clone, or pass --changed-from / CW_ONRAMP_BASE.`
    );
  }
...
function gitLines(cwd: string, args: string[]): string[] {
  const result = spawnSync("git", args, { cwd, encoding: "utf8", stdio: ["ignore", "pipe", "pipe"], timeout: GIT_TIMEOUT_MS });
